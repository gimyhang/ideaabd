...
const primaryAuthor = props.book.authors?.find(a => a.pivot?.is_primary) || props.book.authors?.[0];
const coverPreview = ref(props.book.cover_url || '');
const hasPreviewError = ref(false);

const isExternalUrl = props.book.cover_url &&
    props.book.cover_url.startsWith('http') &&
    !props.book.cover_url.includes('ui-avatars.com') &&
    !props.book.cover_url.includes('/storage/');
...
const form = useForm({
    ...
    language: props.book.language || 'bn',
    price: props.book.price || '',
    discount_price: props.book.discount_price || '',
    stock_quantity: props.book.stock_quantity ?? 10,
    edition: props.book.edition || '',
    page_count: props.book.page_count || '',
    published_year: props.book.published_year || '',
    meta_title: props.book.meta_title || '',
    meta_description: props.book.meta_description || '',
    is_active: Boolean(props.book.is_active),
    is_featured: Boolean(props.book.is_featured),
    is_bestseller: Boolean(props.book.is_bestseller),
    cover: null as File | null,
    cover_url: isExternalUrl ? props.book.cover_url : '',
    preview_type: props.book.preview_type || 'none',
    max_preview_pages: props.book.max_preview_pages || 15,
...
<template>
    ...
    <AdminLayout>
        <template #header>Edit {{ book.title }}</template>
        ...
        <div class="max-w-5xl mx-auto space-y-6 font-sans">
            ...
            <div class="flex items-center justify-between p-6 rounded-2xl shadow-sm border transition-colors bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-900 dark:text-white">
                <div>
                    <h1 class="text-2xl font-black font-heading tracking-tight">Edit Book Details</h1>
                    <p class="text-xs mt-1 text-slate-500 dark:text-zinc-400">
                        Update book metadata, pricing snapshots, stock inventory, or sample preview.
                    </p>
                </div>
            ...
            >
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Input v-model="form.slug" label="URL Slug" placeholder="e.g. deyal" :error="form.errors.slug" />
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input v-model="form.sku" label="SKU Code *" placeholder="e.g. BK-DEYAL-001" :error="form.errors.sku" />
                        <Input v-model="form.isbn_13" label="ISBN-13" placeholder="e.g. 9789849501234" :error="form.errors.isbn_13" />
                    </div>
                ...
                <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-zinc-800">
                    <h3 class="text-sm font-bold font-heading uppercase tracking-wider border-b pb-2 text-slate-900 dark:text-white border-slate-100 dark:border-zinc-800">
                        4. Cover Image & Settings
                    </h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        <label class="block text-xs font-bold mb-1.5 text-slate-800 dark:text-slate-200">Upload / Replace Cover Image File</label>
                ...
                <div class="space-y-4 pt-4 border-t border-slate-100 dark:border-zinc-800">
                    <h3 class="text-sm font-bold font-heading uppercase tracking-wider border-b pb-2 text-slate-900 dark:text-white border-slate-100 dark:border-zinc-800 flex items-center gap-2">
                        ...
                        <span class="text-xs normal-case text-slate-400 font-normal">(একটু পড়ে দেখুন)</span>
                    </h3>
                    ...
                    <div v-if="form.preview_type === 'images'" class="p-4 rounded-xl bg-slate-50 dark:bg-zinc-800/60 border border-slate-200 dark:border-zinc-700 space-y-2">
                        <label class="block text-xs font-bold text-slate-800 dark:text-slate-200">Upload / Replace Sample Page Images (Multiple)</label>
                        <input
                            type="file"
                            multiple
                            accept="image/*"
                        ...
                        />
                        <p v-if="book.sample_pages && book.sample_pages.length > 0" class="text-xs text-emerald-600 font-medium">
                            Currently {{ book.sample_pages.length }} sample page image(s) configured.
                        </p>
                    </div>
                    ...
                    <div v-if="form.preview_type === 'pdf'" class="p-4 rounded-xl bg-slate-50 dark:bg-zinc-800/60 border border-slate-200 dark:border-zinc-700 space-y-2">
                        <label class="block text-xs font-bold text-slate-800 dark:text-slate-200">Upload / Replace Sample PDF File (Max 20MB)</label>
                        <input
                            type="file"
                            ...
                            class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-rose-50 file:text-rose-700 hover:file:bg-rose-100 cursor-pointer"
                        />
                        <p v-if="book.sample_pdf" class="text-xs text-emerald-600 font-medium">
                            Currently sample PDF configured in private storage.
                        </p>
                    </div>
                </div>
