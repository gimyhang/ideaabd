...
@section('title', 'Edit Campaign: ' . $campaign->title . ' — ideaabd')

@section('content')
@php
    $fSettings = $campaign->form_settings ?? [];
    $stdFields = $fSettings['fields'] ?? [
        'name'        => ['enabled' => true, 'label' => 'পূর্ণ নাম (Full Name)'],
        'phone'       => ['enabled' => true, 'label' => 'মোবাইল নম্বর (Phone)'],
        'email'       => ['enabled' => true, 'label' => 'ইমেইল ঠিকানা (Email)'],
        'institution' => ['enabled' => true, 'label' => 'প্রতিষ্ঠান / পেশা (Institution / Org)'],
        'location'    => ['enabled' => true, 'label' => 'জেলা ও থানা (District & Thana)'],
        'address'     => ['enabled' => true, 'label' => 'পূর্ণ ঠিকানা (Full Address)'],
    ];
@endphp
...
<div class="container-fluid px-3 px-md-4 py-4">
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1 small">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.event-campaigns.show', $campaign->id) }}" class="text-decoration-none text-muted">{{ $campaign->title }}</a></li>
                    <li class="breadcrumb-item active text-primary" aria-current="page">Edit</li>
                </ol>
            </nav>
            <h1 class="h3 fw-bold mb-0 text-gray-900">Edit Campaign</h1>
        </div>
        <div class="d-flex align-items-center gap-2">
            <a href="{{ $campaign->public_url }}" target="_blank" class="btn btn-outline-primary rounded-pill px-3 py-1.5 small fw-semibold">
                <i class="fa-solid fa-arrow-up-right-from-square me-1"></i> View Live
            </a>
            <a href="{{ route('admin.event-campaigns.show', $campaign->id) }}" class="btn btn-outline-secondary rounded-pill px-3 py-1.5 small">
    ...
    <form action="{{ route('admin.event-campaigns.update', $campaign->id) }}" method="POST" enctype="multipart/form-data" id="campaignForm">
        @csrf
        @method('PUT')
        <input type="hidden" name="custom_fields_json" id="custom_fields_json" value="{{ old('custom_fields_json', json_encode($campaign->custom_fields ?? [])) }}">
        <input type="hidden" name="form_settings_json" id="form_settings_json" value="{{ old('form_settings_json', json_encode($fSettings)) }}">
            ...
            <div class="col-lg-8">
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="campaignTitle" class="form-control" value="{{ old('title', $campaign->title) }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="slug" id="campaignSlug" class="form-control font-monospace" value="{{ old('slug', $campaign->slug) }}" required style="font-size: 14px;">
                        </div>
                        <div class="d-flex align-items-center justify-content-between mt-1">
                            <small class="text-muted" style="font-size: 11.5px;">Direct URL: English letters, numbers, hyphens.</small>
                            <span id="slugPreview" class="badge bg-light text-dark border font-monospace" style="font-size: 11px;">Preview: {{ url('/') }}/{{ $campaign->slug }}</span>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="type" class="form-select" required>
                                ...
                                <option value="competition" {{ old('type', $campaign->type) == 'competition' ? 'selected' : '' }}>Competition</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="badge_text" class="form-control" placeholder="e.g. Open, Registration Live, 2026" value="{{ old('badge_text', $campaign->badge_text) }}">
                        </div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-dark small">Short Summary</label>
                        <textarea name="short_description" rows="2" class="form-control">{{ old('short_description', $campaign->short_description) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-dark small">Full Description & Rules</label>
                        <textarea name="description" rows="6" class="form-control">{{ old('description', $campaign->description) }}</textarea>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if($campaign->banner_image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $campaign->banner_image) }}" alt="Current Banner" class="rounded-3 img-fluid border" style="max-height: 120px; object-fit: cover;">
                            </div>
                        @endif
                        ...
                        <table class="table table-hover align-middle mb-0" style="font-size: 13px;">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="name" value="{{ $stdFields['name']['label'] ?? 'পূর্ণ নাম (Full Name)' }}" placeholder="পূর্ণ নাম">
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="phone" value="{{ $stdFields['phone']['label'] ?? 'মোবাইল নম্বর (Phone)' }}" placeholder="মোবাইল নম্বর">
                                    </td>
                                    <td class="pe-3 text-end">
                                        <span class="badge bg-success-subtle text-success">Required</span>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="email" value="{{ $stdFields['email']['label'] ?? 'ইমেইল ঠিকানা (Email)' }}" placeholder="ইমেইল">
                                    </td>
                                    <td class="pe-3 text-end">
                                        <div class="form-check form-switch d-inline-block m-0">
                                            <input class="form-check-input std-field-enable" type="checkbox" data-key="email" {{ !empty($stdFields['email']['enabled']) ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="institution" value="{{ $stdFields['institution']['label'] ?? 'প্রতিষ্ঠান / পেশা (Institution / Org)' }}" placeholder="প্রতিষ্ঠান / পেশা">
                                    </td>
                                    <td class="pe-3 text-end">
                                        <div class="form-check form-switch d-inline-block m-0">
                                            <input class="form-check-input std-field-enable" type="checkbox" data-key="institution" {{ !empty($stdFields['institution']['enabled']) ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="location" value="{{ $stdFields['location']['label'] ?? 'জেলা ও থানা (District & Thana)' }}" placeholder="জেলা ও থানা">
                                    </td>
                                    <td class="pe-3 text-end">
                                        <div class="form-check form-switch d-inline-block m-0">
                                            <input class="form-check-input std-field-enable" type="checkbox" data-key="location" {{ !empty($stdFields['location']['enabled']) ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm std-field-label" data-key="address" value="{{ $stdFields['address']['label'] ?? 'পূর্ণ ঠিকানা (Full Address)' }}" placeholder="পূর্ণ ঠিকানা">
                                    </td>
                                    <td class="pe-3 text-end">
                                        <div class="form-check form-switch d-inline-block m-0">
                                            <input class="form-check-input std-field-enable" type="checkbox" data-key="address" {{ !empty($stdFields['address']['enabled']) ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="has_fee_or_donation" value="1" id="feeSwitch" onchange="toggleFeeSection(this.checked)" {{ old('has_fee_or_donation', $campaign->has_fee_or_donation) ? 'checked' : '' }}>
                        <label class="form-check-label fw-semibold text-dark" for="feeSwitch">
                            Enable Fee or Donation
                        </label>
                    </div>

                    <div id="feeSettingsSection" class="{{ old('has_fee_or_donation', $campaign->has_fee_or_donation) ? '' : 'd-none' }}">
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-dark small">Fixed Fee (৳)</label>
                                <input type="number" step="0.01" name="fee_amount" class="form-control" placeholder="0.00" value="{{ old('fee_amount', $campaign->fee_amount) }}">
                            </div>
                            <div class="col-md-6">
                                <div class="form-check form-switch mt-4 pt-2">
                                    <input class="form-check-input" type="checkbox" name="is_donation_flexible" value="1" id="flexibleDonationSwitch" {{ old('is_donation_flexible', $campaign->is_donation_flexible) ? 'checked' : '' }}>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="min_donation" class="form-control" placeholder="10.00" value="{{ old('min_donation', $campaign->min_donation) }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-dark small">Payment Channels</label>
                                <input type="text" name="payment_methods" class="form-control" placeholder="bKash, Nagad, Rocket, Bank" value="{{ old('payment_methods', $campaign->payment_methods) }}">
                            </div>
                        </div>

                        <div class="mb-2">
                            <label class="form-label fw-semibold text-dark small">Payment Instructions</label>
                            <textarea name="payment_instructions" rows="2" class="form-control" placeholder="e.g. Send Money to bKash Personal 01XXXXXXXXX and enter TrxID...">{{ old('payment_instructions', $campaign->payment_instructions) }}</textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="d-flex align-items-center gap-2 mb-3">
                        <input type="color" name="theme_color" id="themeColorInput" class="form-control form-control-color border-0 rounded-3 p-1" value="{{ old('theme_color', $campaign->theme_color ?: '#0284c7') }}" title="Choose accent color" style="width: 48px; height: 38px;">
                        <input type="text" id="themeColorHex" class="form-control font-monospace" value="{{ old('theme_color', $campaign->theme_color ?: '#0284c7') }}" oninput="syncColor(this.value)">
                    </div>
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" name="starts_at" class="form-control" value="{{ old('starts_at', $campaign->starts_at ? $campaign->starts_at->format('Y-m-d\TH:i') : '') }}">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-dark small">Ends At</label>
                        <input type="datetime-local" name="ends_at" class="form-control" value="{{ old('ends_at', $campaign->ends_at ? $campaign->ends_at->format('Y-m-d\TH:i') : '') }}">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-dark small">Max Quota</label>
                        <input type="number" name="max_participants" class="form-control" placeholder="Leave blank for unlimited" value="{{ old('max_participants', $campaign->max_participants) }}">
                    </div>

                    <div class="form-check form-switch mb-1">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" id="activeSwitch" {{ old('is_active', $campaign->is_active) ? 'checked' : '' }}>
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="contact_phone" class="form-control" placeholder="01558712810" value="{{ old('contact_phone', $campaign->contact_phone) }}">
                    </div>

                    <div class="mb-2">
                        <label class="form-label fw-semibold text-dark small">Contact Email</label>
                        <input type="email" name="contact_email" class="form-control" placeholder="support@ideaabd.com" value="{{ old('contact_email', $campaign->contact_email) }}">
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-xs rounded-4 bg-white p-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="success_message" rows="2" class="form-control">{{ old('success_message', $campaign->success_message) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-dark small">Redirect URL</label>
                        <input type="url" name="redirect_url" class="form-control" placeholder="https://..." value="{{ old('redirect_url', $campaign->redirect_url) }}">
                    </div>
...
try {
    const rawVal = document.getElementById('custom_fields_json').value;
    if (rawVal) {
        customFields = JSON.parse(rawVal);
    }
} catch (e) {
    customFields = [];
}
