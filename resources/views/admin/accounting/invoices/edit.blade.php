            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="customer_name" class="form-control @error('customer_name') is-invalid @enderror" 
                           placeholder="Client / Contact person name..." value="{{ old('customer_name', $invoice->customer_name) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="customer_org" class="form-control" 
                           placeholder="Library, Bookshop or Company name..." value="{{ old('customer_org', $invoice->customer_org) }}">
                </div>
                <div class="col-md-3">
                ...
                <table class="table table-bordered align-middle mb-0" id="itemsTable" style="min-width: 1100px;">
                    ...
                    <tbody id="itemsBody">
                        @php
                            ...
                            if (empty($items)) {
                                $items = [['title' => '', 'item_type' => 'Book (Hardcover)', 'unit' => 'কপি', 'quantity' => 1, 'unit_price' => 0, 'subtotal' => 0]];
                            }
                        @endphp
                            ...
                            <tr class="item-row" data-row="{{ $i }}">
                                ...
                                <td>
                                    <input type="text" name="items[{{ $i }}][author_name]" class="form-control item-author" 
                                           placeholder="Author / Spec" value="{{ $authorName }}" autocomplete="off">
                                </td>
                                <td>
                                    <select name="items[{{ $i }}][item_type]" class="form-select item-type-select" onchange="onTypeChange(this, {{ $i }})">
                                        ...
                                        <option value="Stationery" @selected(($item['item_type'] ?? '') === 'Stationery' || ($item['item_type'] ?? '') === 'স্টেশনারী')>Stationery</option>
                                ...
                                <td>
                                    <input type="text" name="items[{{ $i }}][unit]" class="form-control item-unit text-center font-monospace" 
                                           value="{{ $unitVal }}" placeholder="একক" autocomplete="off">
                                </td>
                                <td>
            ...
            .then(data => {
                if (data && data.total_due > 0) {
                    if (noticeEl) {
                        ...
                        noticeEl.innerHTML = `<span class="text-danger"><i class="fas fa-circle-exclamation me-1"></i>গ্রাহকের পূর্বের মোট বকেয়া: <strong>৳${data.total_due.toFixed(2)}</strong> (${data.invoices_count || data.due_count}টি বিল)</span> <button type="button" class="btn btn-xs btn-warning text-dark fw-bold ms-1 py-0 px-2 rounded-pill shadow-2xs" onclick="applyCustomerDue(${data.total_due})">+ বিলে জের যুক্ত করুন</button>`;
                    }
                } else {
                    if (noticeEl) noticeEl.style.display = 'none';
                }
