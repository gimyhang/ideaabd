...
@section('title', 'বিল সম্পাদনা #' . $bill->bill_no)
@section('heading', 'বিল সম্পাদনা #' . $bill->bill_no)
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('subadmin.bills.index') }}" class="text-decoration-none">বিল তালিকা</a></li>
    <li class="breadcrumb-item"><a href="{{ route('subadmin.bills.show', $bill) }}" class="text-decoration-none">#{{ $bill->bill_no }}</a></li>
    <li class="breadcrumb-item active" aria-current="page">সম্পাদনা</li>
@endsection

@section('actions')
    <a href="{{ route('subadmin.bills.show', $bill) }}" class="btn btn-outline-secondary">
        <i class="fas fa-eye me-1"></i> বিল দেখুন
    </a>
...
<div style="max-width: 1080px;" class="mx-auto">
    ...
    <form method="POST" action="{{ route('subadmin.bills.update', $bill) }}" id="billForm">
        ...
        <div class="row g-4">
            ...
            <div class="col-12 col-md-6">
                <div class="adm-card h-100 p-3 p-md-4">
                    <div class="d-flex align-items-center justify-content-between mb-3 pb-2 border-bottom">
                        ...
                        <span class="badge bg-primary-subtle text-primary border small">বিল #{{ $bill->bill_no }}</span>
                    </div>
            ...
            <div class="col-12 col-md-6">
                <div class="adm-card h-100 p-3 p-md-4">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            ...
                            <select name="payment_method" class="form-select" required>
                                ...
                                <option value="card" @selected(old('payment_method', $bill->payment_method)==='card')>💳 কার্ড (Card)</option>
                            </select>
                        </div>
                        <div class="col-6">
                    ...
                    <div>
                        <label class="form-label small fw-semibold">অতিরিক্ত নোট বা মন্তব্য</label>
                        <input type="text" name="notes" class="form-control form-control-sm" value="{{ old('notes', $bill->notes) }}">
                    </div>
                </div>
            </div>
                            ...
                            <tbody id="itemsBody">
                                @foreach($bill->items ?? [] as $index => $item)
                                    <tr class="item-row" data-index="{{ $index }}">
                                        <td class="text-center text-muted row-num fw-bold">@bn($index + 1)</td>
                                        <td class="position-relative">
                                            <input type="hidden" name="items[{{ $index }}][book_id]" class="book-id-input" value="{{ $item['book_id'] ?? '' }}">
                                            <div class="input-group input-group-sm">
                                                <span class="input-group-text bg-light"><i class="fas fa-search text-muted"></i></span>
                                                <input type="text" name="items[{{ $index }}][title]" class="form-control book-title-input" 
                                                       required value="{{ $item['title'] ?? '' }}" placeholder="বইয়ের নাম বা ISBN লিখুন..." autocomplete="off"
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $index }}][qty]" class="form-control form-control-sm text-center qty-input fw-bold" 
                                                   value="{{ $item['qty'] ?? 1 }}" min="1" required oninput="recalc()">
                                        </td>
                                        <td>
                                            <div class="input-group input-group-sm">
                                                <span class="input-group-text bg-light">৳</span>
                                                <input type="number" name="items[{{ $index }}][price]" class="form-control text-end price-input fw-bold" 
                                                       value="{{ $item['price'] ?? 0 }}" min="0" step="0.01" required oninput="recalc()">
                                        </div>
                                        </td>
                                        <td>
                                            <div class="input-group input-group-sm">
                                                <input type="number" name="items[{{ $index }}][discount_pct]" class="form-control text-center discount-pct-input" 
                                                       value="{{ $item['discount_pct'] ?? 0 }}" min="0" max="100" step="0.5" placeholder="০" oninput="recalc()">
                                                <span class="input-group-text bg-light">%</span>
                                            </div>
        ...
        <div class="mt-4 p-3 adm-card d-flex flex-wrap gap-2 justify-content-between align-items-center">
            <a href="{{ route('subadmin.bills.show', $bill) }}" class="btn btn-outline-secondary px-4">
                <i class="fas fa-arrow-left me-1"></i> বাতিল
            </a>
